...
export async function POST(request) {
  ...
  try {
    ...
    const projectResult = await executeQuery({
      query: "INSERT INTO PROJECTS (project_name, start_date, end_date) VALUES (?, ?, ?)",
      values: [
        projectName,
        projectDuration.split("~")[0].trim(),
        projectDuration.split("~")[1].trim(),
      ],
    });
    ...
    for (const material of materials) {
      ...
      if (
        material.category_code_id == null ||
        material.material_quantity == null ||
        material.material_unit_price == null ||
        material.labor_quantity == null ||
        material.labor_unit_price == null ||
        material.expenses_quantity == null ||
        material.expenses_unit_price == null
      ) {
        throw new Error(
          "Incomplete material data received for project materials."
        );
      }

      await executeQuery({
        query: `INSERT INTO PROJECT_MATERIALS (
